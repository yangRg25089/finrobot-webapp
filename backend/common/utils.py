...
言語指示（Chat用）：
- このチャット上のアシスタントの発話は日本語で行ってください。
- 見出し・要約・説明など、画面に表示する文章は日本語で自然に。

Artifacts（ファイル出力）に関する厳格ルール：
- ディスクに書き出す**すべて**（PDF本文、PDFに組み込むテキスト断片 .txt、図表のタイトル/軸/凡例/キャプション、画像のalt/字幕）は**英語のみ**で作成すること（ASCII優先）。日本語は**ファイルに書かない**。
- チャットで日本語草稿を書いたら、**英語に書き直してから**ファイルに保存すること（例：`*_en.txt`）。
- 文字数チェックは**PDFに入る英語版**で行うこと（日本語草稿で数えない）。
- Ticker/コード識別子/APIフィールド名は英語のまま。
- Markdownはチャット用。日付は YYYY-MM-DD。
- 最終メッセージは "TERMINATE" で終える。
"""
...
语言指示（用于聊天）：
- 本次对话中的助理发言请使用中文，表述自然、清晰。
- 标题/总结/说明等在界面展示的文本使用中文。

有关 Artifacts（文件输出）的硬性规则：
- 任何写入磁盘的**所有内容**（PDF 正文、用于组装 PDF 的 .txt 片段、图表标题/坐标轴/图例/说明、图片字幕/alt）**必须只用英文**（优先 ASCII）。**不要把中文写入文件**。
- 若在聊天中先写了中文草稿，**先改写为英文**再保存到文件（例如使用 `*_en.txt` 后缀）。
- 字数校验以**将进入 PDF 的英文版**为准（不要用中文稿计数）。
- Ticker/代码标识符/API 字段名保持英文。
- Markdown 仅用于聊天展示；日期使用 YYYY-MM-DD。
- 最后一条消息以 "TERMINATE" 结束。
"""
