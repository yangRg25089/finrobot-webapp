    ...
    try:
        ...
        for record in history_records:
            formatted_record = {
                ...
                "generated_files": record.get("generated_files", None),
            }
        ...
        for script_dir in history_base_dir.iterdir():
            ...
            if history_records:
                scripts_overview[script_name] = {
                    ...
                    "latest_timestamp": (
                        history_records[0].get("timestamp", "")
                        if history_records
                        else ""
                    ),
                    "latest_display_name": (
                        format_display_name(history_records[0])
                        if history_records
                        else ""
                    ),
                }

        return {"success": True, "scripts": scripts_overview}

    except Exception as e:
        return {"success": False, "error": str(e), "scripts": {}}


def delete_conversation_history(
    script_name: str, timestamp: str = None
) -> Dict[str, Any]:
    """
    删除对话历史
    ...
    try:
        ...
        if timestamp:
            ...
            if script_history_dir.exists():
                ...
                return {"success": False, "error": "No history found for this script"}
